/*
@Name：WeTalk 自动化签到 (Egern 兼容版)
@Author：iBL3ND修改
@Desc：基于 PingMe 逻辑框架重写，支持 Egern 抓包与自动签到视频奖励。
@Date：2026-04-22

============ Egern 配置 ============
[Script]
WeTalk_Checkin = type=cron,cron="30 8,20 * * *",handler=WeTalk.js
WeTalk_Cookie = type=http-request,pattern=^https:\/\/api\.wetalkapp\.net\/app\/queryBalanceAndBonus,requires-body=false,handler=WeTalk.js

[MITM]
hostname = api.wetalkapp.net
*/

const $ = new Env('WeTalk签到');
const isNode = $.isNode();
const notify = isNode ? require('./sendNotify') : '';

// --- 核心配置 ---
const ckKey = 'wetalk_accounts_v1'; 
const SECRET = '0fOiukQq7jXZV2GRi9LGlO'; // 签名盐值
const API_HOST = 'api.wetalkapp.net';
const MAX_VIDEO = 5;
...
async function startTasks() {
    ...
    if (typeof $request !== 'undefined') {
        ...
        $.msg($.name, '✅ WeTalk 参数获取成功', '请关闭重写并运行定时任务');
        return;
    }
    ...
    if (!raw) {
        await sendMsg("❌ 未获取到参数", "请先开启脚本并在 WeTalk APP 中触发请求");
        return;
    }
    ...
    try {
        ...
        $.log(`运行时异常: ${err}`);
        await sendMsg("❌ 任务失败", err.message || String(err));
    }
}
...
async function sendMsg(desc, opts) { 
    $.isNode() ? await notify.sendNotify($.name, desc) : $.msg($.name, "", desc, opts);
}
